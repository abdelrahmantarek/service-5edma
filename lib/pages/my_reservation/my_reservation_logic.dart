...
class MyReservationLogic extends GetxController {
  ...
  Future getMyReservation() async{
    loading.value = true;
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection("Reservation").where("user.id",isEqualTo: Get.find<UserData>().id).get();
    list.clear();
